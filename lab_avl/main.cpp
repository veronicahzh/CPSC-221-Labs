 ...
 *  - CRTP tree printing
 *  - doxygen
 * Modified by William Zhang in Summer 2022.
 */
...
void printAfterRemove(int removed)
{
    cout << endl << "~";
    colored_out::output_bold("After remove(");
    colored_out::output_bold(removed);
    colored_out::output_bold(")");
    cout << "~" << endl;
}
...
{
  AVLTree<int, int> tree;
  printHeader("Removal Case 2");
  for (int k : {9,5,11,4,6,10,12,3}) {
    tree.insert(k, k);
  }
  printBefore();
  tree.print();
