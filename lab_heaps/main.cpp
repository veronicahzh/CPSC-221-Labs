...
void printHeader(const string& headline) {
    ...
    cout << endl
         << string(64, colored_out::BORDER_CHAR) << endl;
}
...
vector<int> setUpVector(int seed, int count) {
    ...
    for (int i = 0; i < count; i++)
        result.push_back(i);
    rand_gen.shuffle(result.begin(), result.end());
    return result;
}
...
int main(int argc, char** argv) {
    ...
    if (is_colored) {
        ...
        else coloroutput.set_expected_file("soln_heapfun_1.out");
    ...
    for (int i = 0; i < COUNT_FULL; i++)
        myArray.push_back(i);
    rand_gen.shuffle(myArray.begin(), myArray.end());
